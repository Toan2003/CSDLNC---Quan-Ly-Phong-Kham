...
--LẤY CÁC DANH MỤC
CREATE FUNCTION LAYDANHMUC()
RETURNS TABLE
AS
	RETURN (SELECT DM.MADANHMUC, DM.TENDM
			FROM DANHMUCDIEUTRI DM)
GO

--1 LẤY DANH SÁCH BUỔI ĐIỀU TRỊ THEO BỆNH NHÂN VÀ SẮP XẾP THEO NGÀY
...
--2 LẤY DANH SÁCH BUỔI ĐIỀU TRỊ TỪ NGÀY A->B
...
EXEC LAYBUOIDT_NGAY '2023-12-1','2023-12-1'
GO
...
--3. XEM THÔNG TIN CHI TIẾT CỦA 1 BUỔI ÐIỀU TRỊ
...
=======
		...
		--LẤY THÔNG TIN CHI TIẾT LOẠI ĐIỀU TRỊ
		SELECT CTDT.MADIEUTRI, LDT.TENDIEUTRI, LDT.MADANHMUC
		FROM  BUOIDIEUTRI BDT
			JOIN CHITIETDIEUTRI CTDT ON BDT.IDBUOIDIEUTRI = CTDT.IDBUOIDIEUTRI
			JOIN LOAIDIEUTRI LDT ON CTDT.MADIEUTRI = LDT.MADIEUTRI
		WHERE BDT.IDBUOIDIEUTRI = @IDBUOIDIEUTRI
		
		--LẤY THÔNG TIN CHI TIẾT RĂNG ĐIỀU TRỊ
		SELECT CTRDT.MADIEUTRI, CTRDT.TENRANG, CTRDT.MATDIEUTRI
		FROM  CHITIETRANGDIEUTRI CTRDT 
		WHERE CTRDT.IDBUOIDIEUTRI = @IDBUOIDIEUTRI
...
--4 XEM CHI TIẾT KẾ HOẠCH ĐIỀU TRỊ
...
BEGIN TRAN
	BEGIN TRY
		...
		IF (@IDDIEUTRI IS NULL OR @BENHNHAN IS NULL OR @BSPHUTRACH IS NULL OR @TRANGTHAI IS NULL)
			...
			END
		--THỰC THI
		INSERT KEHOACHDIEUTRI(IDDIEUTRI,MOTAKHDT,TRANGTHAI,GHICHUKHDT,TONGGIA,BENHNHAN,BSPHUTRACH)
		VALUES
			( @IDDIEUTRI,@MOTAKHDT,@TRANGTHAI, @GHICHUKHDT, @TONGGIA, @BENHNHAN, @BSPHUTRACH)
	END TRY
	BEGIN CATCH
		PRINT N'LỖI HỆ THỐNG'
		ROLLBACK TRAN
